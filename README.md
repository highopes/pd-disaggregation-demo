...
| csco-k8s-03 | 192.168.160.113 | ComfyUI；Dynamo Frontend 为 CPU Pod | ens65np0 / mlx5_0:1 / 172.31.230.113 | Eth1/2/1 |
| csco-k8s-04 | 192.168.160.114 | foundation-instruct-vllm，1 × L4 | ens65np1 / mlx5_0:1 / 172.31.230.114 | Eth1/2/2，物理链路原本 down |
...
Backend 是专用 L2 VLAN 2310 `DYNAMO_ROCE_BACKEND`，子网 `172.31.230.0/24`，无 gateway、DNS 或默认路由。Linux MTU 为 9000，Nexus 端口与 no-drop queue MTU 为 9216。
...
Nexus 的实际配置、端口映射和回滚分别见 `pd-disaggregation/nexus/final-relevant.cfg`、`cx7-port-mapping.md`、`rollback.cfg`。DSCP 26 分类和 PFC cos 3 是既有策略；本任务创建 VLAN 2310、移动四个 access port，并把 qos-group 3 MTU 从错误的 2240 修正为 9216。
...
保留原 `qwen-openai` Service 名、ClusterIP/NodePort 30080 和 `app: qwen-openai` label，因此现有 ServiceMonitor 继续抓取 `/metrics`。

Service 使用 `externalTrafficPolicy: Local` 保存真实外部源地址；NodePort 应访问运行 Frontend 的 node3，即 `192.168.160.113:30080`。
...
RoCE 基准已实测双向 jumbo DF ping 0 loss，以及带 traffic class 106 的 `ib_write_bw` 109.85 Gbit/s；详情见 [roce-baseline.md](pd-disaggregation/evidence/roce-baseline.md)。
